"""
    Related functionss are modified from
    https://github.com/hughw19/NOCS_CVPR2019
"""

"""

Sampling point clouds for FPS 128.
...
def draw_detections(img, out_dir, data_name, img_id, intrinsics, pred_sRT, pred_size,
    ...
    for i in range(pred_sRT.shape[0]):
        ...
        # xyz_axis = 0.1 * np.array([[0, 0, 0], [1, 0, 0], [0, 1, 0], [0, 0, 1]]).transpose()
        xyz_axis = 0.5 * np.array([[0, 0, 0], [1, 0, 0], [0, 1, 0], [0, 0, 1]]).transpose()
        transformed_axes = transform_coordinates_3d(xyz_axis, sRT)
        projected_axes = calculate_2d_projections(transformed_axes, intrinsics)
...
def draw_sapien_detections(img, out_dir, data_name, img_id, intrinsics, pred_sRT, pred_size,
                    gt_sRT, gt_size, gt_class_ids, nocs_sRT, nocs_size, nocs_class_ids, draw_gt=True, draw_nocs=True):
    """ Visualize pose predictions.
    """
    out_path = os.path.join(out_dir, '{}_{}_pred.png'.format(data_name, img_id))
...
if __name__ == "__main__":
    ...
    # input category & id P.S. Select an instance as a template for each category.
    category = input("please input category: ")
    id = input("please input id: ")

    # read ply to get points P.S. ply-new.ply is generated by meshlab. (flatten parts for a whole instance)
    ply_file = os.path.join("./dataset", id, "ply-new.ply")
    ...
    # save
    poisson_ply_file = os.path.join("./dataset", id, "ply-2000.ply")
    o3d.io.write_point_cloud(poisson_ply_file, pcd)
    points = np.asarray(pcd.points)

    # farthest_point_sampling
    n_samples = 128
    selected_pts = farthest_point_sampling(points, n_samples)
    sampled_point_cloud = points[selected_pts]

    # save to obj
    obj_file = "template_128.obj"
    obj_path = os.path.join("./render", category, obj_file)

    # debug
    # with open(obj_file, "wb") as obj_file:
    #     pickle.dump(selected_pts, obj_file)

    with open(obj_path, 'w') as f:
        for point in sampled_point_cloud:
            # obj format
            f.write(f"v {point[0]} {point[1]} {point[2]}\n")
    
    print(f"Saved to {obj_path} successfully!")


    # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ #
    # Above for mesh-flattened, Below for ply-10000.ply
    # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ #
    ...
    # # read ply to get points
    # ply_file = os.path.join("./dataset", id, "point_sample/ply-10000.ply")
